...
struct PSTnode {
    int lson,rson,sum;
}PST[maxn << 5]; //一个结构体元素代表一个主席树结点，PST[i]维护的是编号为i的结点的信息,lson是其左孩子结点的编号，rson是其右孩子结点的编号，sum维护的是区间信息(区间和)。
int cnt,root[maxn];// cnt是内存池尾标，表明当前已经开辟了几个结点,root[i]是第i个版本的根结点的编号
```

PST开辟`32maxn`的空间足够(int规模数据下log值最多为32)。
...
// insert(1,n,root[i-1],root[i],num); -- 注意num是离散化后的值,注意n是离散化后的长度
```
...
```c++
int query(int l,int r,int pre,int now,int k){ //pre为1~x-1的版本，now为1~y的版本
    if(l == r) return l;
    int mid = (l+r)>>1;
    
    //查询左区间的值，意思就是查询[L,R]区间有多少个数小于等于mid
    int tot = PST[ PST[now].lson ].sum - PST[ PST[pre].lson ].sum;//相减即当前区间维护的值
    if(k <= tot) return query(l,mid,PST[pre].lson,PST[now].lson,k);// 共同去向左子树
    return query(mid+1,r,PST[pre].rson,PST[now].rson, k - tot); // 共同去向右子树
}
// int ans = query(1,n,root[x-1],root[y],k); -- 注意n是离散化后的长度而非原长度
```
...
void build(int l,int r,int &now){
    ...
    now = cnt;
    if(l == r){
        pst[now].v = a[l]; // 具体情况具体分析
        return ;
    }
    int mid = (l + r) >> 1;
    build(l,mid,pst[now].lson);
    build(mid+1,r,pst[now].rson);
}

// build(1,n,root[0]); -- 假定初始版本编号为0
```
...
首先重建线性基，尽力把每个$p[i]$只留下第$i$位的$1$，从而使得各个位之间互不影响，也就是说它的理想效果如下： $p[0] 0 0 0 0 1 p[1] 0 0 0 1 0 p[2] 0 0 1 0 0 p[3] 0 1 0 0 0 p[4] 1 0 0 0 0$.

```c++
LL a[MAXBIT + 5]; // 原线性基
LL d[MAXBIT + 5]; // 记录重建的线性基
LL cnt; //重建线性基尾标
void rebuild(){
    for(int i=MAXBIT;i>=0;i--){
        for(int j=i-1;j>=0;j--){
...
紧接着，可得异或第$k$小，第$1$小就是$d[0]$，第2小就是$d[1]$，第3小就是$d[0]\  xor\  d[1]$，第4小就是$d[2]$，第五小就是$d[2]\ xor\ d[0]$，第六小就是$d[2]\ xor\ d[1]$，以此类推，可发现，这正好对应k的二进制位数。但要注意特判$0$的情况:

```c++
LL d[MAXBIT + 5]; // 记录重建的线性基
LL cnt; //重建线性基尾标
bool flag; //记录原先的数字能否异或出0
//查询异或第k小，若不存在返回-1
LL getkth(LL k){
    if(flag) k--; //如果包含0,要考虑0
    if(k >= (1LL << cnt)) return -1; //最多只有(1<<cnt) - 1种结果
    LL ans = 0;
    for(int i=MAXBIT;i>=0;i--){
        if(k & (1LL << i)) ans ^= d[i]; //k的二进制第i位为1，就要异或d[i]
    }
    return ans;
}
```
...
int getrank(LL x){
    ...
    for(int i=cnt-1;i>=0;i--){
        if(x >= d[i]){ // x比d[i]大，那么从d[1] ~ d[i-1]异或出的所有结果都比x小
            res += (1 << i);
            x ^= d[i];
        }
    }
    return res + flag; // 注意考虑0的贡献
}
```
...
Manacher用于统计字符串中最长回文对称串，时间复杂度稳定为$O(n)$。

先将字符串的两两字符之间加上`#`字符以将所有的对称串的长度变为奇数。为防止边界出现错误，将字符串两头加上从未出现过的字符(比如`@`)。

$d[i]$ 表示以$i$为中心的最长对称串的半径，注意$d[i] ≥ 1$，当$d[i] == 1$​​时，即仅与自身对称。

![](picture/20.png)

```c++
int d[maxn << 1]; // d[i]表示以i为中心的最长对称半径,这里指的是变长的串的下标，所以注意开两倍空间
int ans[maxn]; // ans[i] 表示原串中以i结尾的最长回文串长度
void SOLVE(){
    string str;
    cin >> str; // 读入原串
    string s; // 记录加了#号的变长串
    s += "$#"; // s[0] == '$' s[1] == '#'
    for(auto x : str){ // 交替添加字符和#号
        s.push_back(x); // 添加字符
        s += "#"; // 添加'#'
    }
    int n = s.length(); // 变长串长度
    
    d[1] = 1; // 初始化
    int l,r = 1; // l,r记录当前盒子左右端点，也就是右端点最靠右的最长回文串
    for(int i=2;i<=n;i++){
        if(i <= r) d[i] = min(d[r - i + l],r - i + 1);
        while(s[i - d[i]] == s[i + d[i]]) d[i]++;
        if(i + d[i] - 1 > r){
            // >>>> 这里是记录ans的部分，即获得原串中所有位置的回文长度，具体问题若不要求则可不写
            for(int j=r+1;j<=i+d[i]-1;j++){ // 从上一个盒子的右端点到当前更新的盒子的右端点之间是新的未处理的位置，跑一遍，得到所有位置的回文长度
                if(s[j] != '#' and s[j] != '$'){ // 当前位置是原字符位置
                    ans[j / 2] = j - i + 1; // 当前的j位置对应的是原串的j/2位置(原串下标从1开始)
                }
            }
            // <<<<
            // 更新盒子左右端点
...
如果仅需求最长回文串的长度，那么就是$MAX\{d[i]\} - 1$。即半径减1，因为半径包含#号，正好对应原串的两倍长度。

## 6.8 扩展KMP(Z函数)

---

对于一个长度为`n`的字符串`str`有Z函数：`z[maxn]`。其中`z[i]`表示`str`从下标`i`开始的后缀子串`str[i~n]`与原串`str`的最长公共前缀`LCP`。求解复杂度为：`O(n)`
...
使用**试除法**对某个数 $x$ 执行质因数分解，时间复杂度为 $O(\sqrt{x})$.

```c++
void get_prime_factor(int x){
    vector<int>res; //记录质因数分解的结果,每个质因数只记录一次
    for(int i=2;i*i<=x;i++){
        if(x % i == 0){
            res.push_back(i);
            while(x % i == 0) x /= i;
    ...
    if(x > 1) res.push_back(x); // 大于√x的质因数最多只有一个，如果最后x不为1，那么一定是这个质因数
}
```

## 8.11 欧拉函数

$1...N$ 中与 $N$ 互质的数的个数，被称为**欧拉函数**，记作$\varphi(N)$。

一个显然的结论是如果 $N$ 是质数，那么$\varphi(N) = N - 1$。因为所有小于 $N$ 的数都和 $N$​ 互质。

**欧拉函数**是积性函数，意味着对于互质的$m,n$，有$\varphi(m \times n) = \varphi(m) \times \varphi(n)$。

对于任何 $N$ ，有：
$$
\varphi(N) = N \times \prod_{p|N} (\frac{p-1}{p})  \ \ \ \ \ \ \ \ \ \ \ \ (p是质数)
$$
...
$p$是质数，那么$p|N$就意味着$p$是$N$的质因子，因此可使用**试除法**在求解$N$的质因数的同时求解$\varphi(N)$。时间复杂度为$O(\sqrt{N})$.
...
long long getPhi(long long N){
    ...
    for(int i=2;i*i<=N;i++){
        if(N % i == 0){
            res = res / i * (i - 1); // i是N的质因子,也就意味着N一定可以除以i，故不需担心除不尽的问题
            while(N % i == 0) N /= i;
        }
    }
...
线性筛法可以求解 $[1,N]$范围内的所有数的欧拉函数，时间复杂度为$O(N)$.

在线性筛中，对于质数可以直接得到其欧拉函数$\varphi(N) = N - 1$；如果是合数$m$,那么$m$其一定是被其**最小的质因子**筛掉的，这里不妨设$m$的最小质因子为$p_m$，那么有$p_m \times i = m$，有两种情况：

1. $p_m$能整除$i$，意味着$i$也有质因子$p_m$，意味着$m$有的质因子$i$也有，那么有：

$$
\varphi(m) = m \times \prod_{p|m} (\frac{p-1}{p}) = p_m \times i \times \prod_{p|m} (\frac{p-1}{p}) = p_m \times (i \times \prod_{p|i} (\frac{p-1}{p})) = p_m \times \varphi(i)
$$

2. $p_m$不能整除$i$,因为$p_m$是质数，所以$p_m$和$i$互质，那么根据欧拉函数的积性函数性质，有:

$$
\varphi(m) = \varphi(p_m \times i) = \varphi(p_m) \times \varphi(i) = (p_m -1) \times \varphi(i)
$$
...
```c++
const double pi = acos(-1.0); // π值
const double inf = 1e100; // 极大值
const double eps = 1e-8; // 误差限缺省
```

## 9.2 符号判断函数 -sgn

```c++
// 判断d值的符号，如果为0则返回0，正值则返回1，负值则返回-1
int sgn(double d){
...
α×β=|α||β|sinθ θ表示向量α旋转到向量β所经过的夹角 对加法满足分配律(不满足交换律) 

几何意义:向量α与β所张成的平行四边形的有向面积 判断外积的符号 

右手定则 α×β 若β在α的逆时针方向，则为正值、顺时针则为负值、两向量共线则为0
...
```c++
double Angle(Vector A, Vector B){return acos(Dot(A, B) / Length(A) / Lenth(B));}
```

## 9.10 计算向量逆时针旋转后的向量 -Rotate
...
// 对于A，B两点构成的直线, line_ab = Line(A - B,A); 直线上两点相减即构成方向向量
```

## 9.15 判断点是否在直线上

利用三点共线的等价条件α×β==0 直线上取两不同点与待测点构成向量求叉积是否为零来判断点是否在直线上（叉积为0互相平行） 判断点与直线的关系（见9.13）

## 9.16 计算两直线交点 -Get_line_intersection
...
## 9.17  计算点到直线的距离 -Distance_point_to_line
...
## 9.18 计算点到线段的距离 -Distance_point_to_segment
...
## 9.19 求点在直线上的投影点 -Get_line_projection
...
## 9.20 判断点是否在线段上 -OnSegment

```c++
bool OnSegment(Point p, Point a1, Point a2){
    return dcmp(Cross(a1-p, a2-p)) == 0 && dcmp(Dot(a1-p, a2-p)) <= 0;// 紫书上写的是小于，但是真正写题的时候写成才A
}
```
...
## 9.21 判断两线段是否相交 -Segment_proper_intersection
...
## 9.22 判断点是否在线段上(不含端点) -on_segment
...
## 9.23 求多边形面积 -convex_polygon_area
...
## 9.24 判断点在多边形内 -is_point_in_polygon
...
特殊地，三角形重心为 $(\frac{x1+x2+x3}{3},\frac{y1+y2+y3}{3})$
...
## 9.28 圆与直线交点 -getLineCircleIntersection

```c++
// 求圆与直线交点
// 交点个数有0,1,2三种可能，sol存储所有交点，函数返回交点的个数
...
## 9.29  求两圆交点 -get_circle_circle_intersection

```c++
// 两圆相交保存所有交点返回交点个数（至多两个) 如果两圆完全重合，返回-1
...
情况一和情况二没什么需要求的，情况三和情况五中的内公切线都对应 于“过圆上一点求圆的切线”，只需连接圆心和切点，旋转90°后即可知道切线的方向向量。这样，问题的关键是求出情况四、五中的外公切线和情况六中的内公切线。

```c++
// 返回切线的条数，-1表示无穷条切线
// a[i]和b[i] 分别是第i条切线在圆A和圆B上的切点,下标均从0开始存储
...
## 9.33 判直线和圆相交 -intersect_line_circle
...
## 9.34 判线段和圆相交 -intersect_seg_circle
...
## 9.35 判圆和圆相交 -intersect_circle_circle
...
## 9.37 求圆外一点对圆的两个切点 -TangentPoint_PC
...
## 9.38 求三角形的外接圆 -get_circumcircle
...
读入一行字符串到string中(即不以空格作为分割字符),例如读入:`abc defglsadlk sd`。

使用**\<string\>**下的**getline**函数实现，如下：

```c++
#include <string>

std::string str;
std::getline(std::cin,str); // 读入一整行字符串

std::cout << str; //abc defglsadlk sd
```

## 11.7 多线程优化thread(C++11+)
